...
bool testWaitMovesJobOnce() {
  ...
  const Result first = game.apply(Action::WAIT);
  const bool movedToB = game.currentJob() != 0 &&
                        strcmp(game.currentJob()->name, "Job B") == 0;
  const Result second = game.apply(Action::WAIT);
  const bool returnedToA = game.currentJob() != 0 &&
                           strcmp(game.currentJob()->name, "Job A") == 0;
  const Result third = game.apply(Action::WAIT);

  return first.accepted && second.accepted && movedToB && returnedToA &&
         !third.accepted && third.reason == Reason::ALREADY_WAITED &&
         game.queueSize() == 2;
}

bool testCancelAppliesPenalty() {
  const Facility facility = {true, true, 22, 28};
  const Job jobs[] = {{"Job A", 2, 200000, 50000, true}};
  Game game;
  const bool started = game.begin(facility, jobs, 1);
  const Result result = game.apply(Action::CANCEL);
...
bool testTotalAcrossTwoJobs() {
  ...
  const Result completed = game.apply(Action::RUN);
  const Result cancelled = game.apply(Action::CANCEL);
  return completed.accepted && cancelled.accepted &&
         cancelled.totalCents == 80000 && game.queueSize() == 0 &&
         game.apply(Action::RUN).reason == Reason::QUEUE_EMPTY;
}
